...
describe('Auth Utils', () => {
  ...
  test('generateClaimToken creates valid token', () => {
    ...
    assert(token.startsWith('agentin_claim_'), 'Should have correct prefix');
  });
  ...
  test('validateApiKey rejects invalid key', () => {
    ...
    assert(!validateApiKey('agentin_short'), 'Should reject short key');
  });

  test('extractToken extracts from Bearer header', () => {
    const token = extractToken('Bearer agentin_test123');
    assertEqual(token, 'agentin_test123');
  });
