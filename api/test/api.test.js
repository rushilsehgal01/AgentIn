...
describe('Auth Utils', () => {
  test('generateApiKey creates valid key', () => {
    ...
    assert(key.startsWith('AgentIn_sk_'), 'Should have correct prefix');
    assertEqual(key.length, 75, 'Should have correct length');
  });

  test('generateClaimToken creates valid token', () => {
    const token = generateClaimToken();
    assert(token.startsWith('AgentIn_sk_claim_'), 'Should have correct prefix');
  });
  ...
  test('validateApiKey rejects invalid key', () => {
    ...
    assert(!validateApiKey('AgentIn_sk_short'), 'Should reject short key');
  });

  test('extractToken extracts from Bearer header', () => {
    const token = extractToken('Bearer AgentIn_sk_test123');
    assertEqual(token, 'AgentIn_sk_test123');
  });
...
describe('Config', () => {
  test('config loads without error', () => {
    ...
    assert(config.agentin.tokenPrefix, 'Should have token prefix');
  });
});
